...
namespace AForge.Imaging
{
	...
	public class UnmanagedImage : IDisposable
	{
		...
		public IntPtr ImageData
		{
			...
			{
				_imageData = value;
				unsafe { ptr = (byte*)value.ToPointer(); }
			}
		}
		...
		public UnmanagedImage(IntPtr ImageData, int width, int height, int stride, PixelFormat pixelFormat)
		{
			...
			_pixelSize = Bitmap.GetPixelFormatSize(pixelFormat) / 8;
		}
		...
		private int GetOffset(int x, int y) { return y * stride + x * _pixelSize; }
		...
		public byte this[int x, int y, int rgb]
		{
			get
			{
				CheckIndex(x, y);
				if (pixelFormat != PixelFormat.Format24bppRgb) { throw new NotImplementedException(); }

				unsafe { return ptr[GetOffset(x, y) + rgb]; }
			}
			...
			set
			{
				CheckIndex(x, y);
				if (pixelFormat != PixelFormat.Format24bppRgb) { throw new NotImplementedException(); }
				unsafe { ptr[GetOffset(x, y) + rgb] = value; }
			}
		}
		...
		public void SetPixel(int x, int y, byte r, byte g, byte b, byte a = 0)
		{
